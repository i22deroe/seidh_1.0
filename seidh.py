...
from lib.constants.constants import proteins,peptides,predictions
from sd.driver import SeidhAdapt
from sd.scoring import seidh_calculate_energy
from lib.constants.constants import MAX_PEPTIDES,MIN_CONTACT,MAX_AA
...
def seidh(protein,peptide,fasta,NUM_PROC=1):
	...
	with open(str(NUM_PROC)+'_cores_simmulation_summary_'+str(datetime.now().strftime("%Y%m%d-%H%M%S")), 'w') as output:
		if clusters > 1:
